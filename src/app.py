...
from .modules.frequenciesChart.api import (
    app as frequenciesChart_Routes,
    get_frequencies_chart,
)
...
from .modules.events.api import (
    app as events_Routes,
    get_rr_intervals,
    get_heart_rate,
    get_beat_classification,
    get_spectral_analysis,
)
from .modules.segmentation_st.api import (
    app as segmentation_st_Routes,
    get_segmentation_st,
)
from .modules.arritmiasDetector.api import (
    app as arritmiasDetector_Routes,
    get_arrhythmias,
)
    ...
    try:
        ...
        modules = [
            ...
            (
                "frequencies",
                get_frequencies_chart,
                {"UPLOAD_DIR": temp_dir, "frequency": base_frequency},
            ),
        ...
        for step, (module_name, func, kwargs) in enumerate(modules, 1):
            try:
                ...
                logger.info(
                    f"Módulo {module_name} finalizado em {elapsed_time:.2f} segundos."
                )
        ...
        num_workers = 2
        workers = [
            asyncio.create_task(worker(queue, client)) for _ in range(num_workers)
        ]
...
async def analyze_ecg(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Erro na conversão do arquivo: {str(e)}"
        )
